...
char PushBackInput_PutCh (FIO_File f, char ch)
{
  if (StackPtr < MaxPushBackStack)
    ...
    {
      Debug_Halt ((char *) "max push back stack exceeded, increase MaxPushBackStack", 55, 125, (char *) "../../gm2-floppsie/gcc/m2/gm2-libs/PushBackInput.mod", 52);
    }
  return ch;
  /* static analysis guarentees a RETURN statement will be used before here.  */
  __builtin_unreachable ();
  ...
  while (l > 0)
    {
      ...
      if ((PushBackInput_PutCh (f, a[l])) != a[l])
        {
          Debug_Halt ((char *) "assert failed", 13, 107, (char *) "../../gm2-floppsie/gcc/m2/gm2-libs/PushBackInput.mod", 52);
        }
    }
}
