                             ...
                             int type)
{
  ...
  if (arrayType != old)
    internal_error (
        "[%s:%d]:array declaration has changed canonicalization has failed",
        __FILE__, __LINE__);
                          ...
                          tree lowval, tree highval)
{
  ...
  tree lo = copy_node (m2convert_BuildConvert (
      location, btype, m2expr_FoldAndStrip (lowval), FALSE));
  tree hi = copy_node (m2convert_BuildConvert (
      location, btype, m2expr_FoldAndStrip (highval), FALSE));
