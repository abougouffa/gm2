...
IMPLEMENTATION MODULE devicePy ;
...
FROM SeqFile IMPORT write, text, read, raw, ChanId, OpenResults, FlagSet ;
...
BEGIN
   ...
   configDevice(initPoint(one(), one()), initPoint(cardinal(Width), cardinal(Height)), FPS)
END Init ;
