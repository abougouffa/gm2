...
/* have we reached a branch yet?  This allows us to
   continue looking after a function call into the next basic block.  */

static bool reached_branch;
...
runtime_exception_inevitable (gimple *stmt)
{
  if (! reached_branch)
	...
	examine_call (stmt);
#if 0
      else if (is_gimple_condexpr (stmt))
	reached_branch = true;
#endif
    }
}
...
pass_warn_exception_inevitable::execute (function *fun)
{
  ...
  FOR_EACH_BB_FN (bb, fun)
	...
	fprintf (stderr, "/* end of bb.  */\n\n");
      count++;
#endif
      reached_branch = true;   /* we can do better than this, but for now it is safe and still useful.  */
      if (reached_branch)
	return 0;
    }
#if defined (DEVELOPMENT)
