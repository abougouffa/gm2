...
VAR
   nargv: ADDRESS ;
BEGIN
   ALLOCATE (nargv, argc * SIZE (ADDRESS)) ;
   nargv := memcpy (nargv, argv, argc * SIZE (ADDRESS)) ;
   RETURN nargv
END dupArgv ;
...
BEGIN
   ...
   n^.argc := high-low+1 ;
   p := o^.argv ;
   INC (p, SIZE (ADDRESS) * low) ;
   ALLOCATE (a, SIZE (ADDRESS) * n^.argc) ;
   n^.argv := memcpy (a, p, SIZE (ADDRESS) * n^.argc) ;
   n^.next := NIL ;
   RETURN n
END Slice ;
...
BEGIN
   ...
   ALLOCATE (result^.argv, result^.argc * SIZE (ADDRESS)) ;
   result^.argv := memcpy (result^.argv, a^.argv, a^.argc * SIZE (ADDRESS)) ;
   result^.argv := memcpy (result^.argv + VAL (ADDRESS, a^.argc * SIZE (ADDRESS)),
                           b^.argv, b^.argc * SIZE (ADDRESS)) ;
   result^.next := NIL ;
   RETURN result
END ConCat ;
