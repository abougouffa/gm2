/* Specific flags and argument handling within GNU Modula-2.
...
static void printOption (const char *desc,
  ...
  if (library != NULL)
    {
      ...
      add_library (build_archive (library), in_decoded_options_count,
                   in_decoded_options, position);
    }
}
...
const struct spec_function lang_specific_spec_functions[] = { { NULL, NULL } };
