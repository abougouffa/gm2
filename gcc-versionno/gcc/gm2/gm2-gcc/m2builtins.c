		  ...
		  tree attrs)
{
  ...
  m2block_pushDecl (decl);
  DECL_BUILT_IN_CLASS (decl) = fclass;
...
unsigned int m2builtins_GetBuiltinTypeInfoType (const char *ident)
{
  int i=0;
...
tree m2builtins_GetBuiltinTypeInfo (location_t location, tree type, const char *ident)
{
  int i=0;
...
static tree doradix (location_t location ATTRIBUTE_UNUSED, tree type)
{
...
static tree doplaces (location_t location ATTRIBUTE_UNUSED, tree type)
{
  if (TREE_CODE (type) == REAL_TYPE)
    {
...
static tree doexponentmin (location_t location ATTRIBUTE_UNUSED, tree type)
{
  if (TREE_CODE (type) == REAL_TYPE)
    {
      enum machine_mode mode = TYPE_MODE (type);
...
static tree doexponentmax (location_t location ATTRIBUTE_UNUSED, tree type)
{
  if (TREE_CODE (type) == REAL_TYPE)
    {
      enum machine_mode mode = TYPE_MODE (type);
...
static tree dolarge (location_t location ATTRIBUTE_UNUSED, tree type)
{
  if (TREE_CODE (type) == REAL_TYPE)
...
static tree dosmall (location_t location ATTRIBUTE_UNUSED, tree type)
{
  if (TREE_CODE (type) == REAL_TYPE)
...
static tree doieee (location_t location ATTRIBUTE_UNUSED, tree type ATTRIBUTE_UNUSED)
{
  /* --fixme-- maybe we should look for the -mno-ieee flag and return this result */
  return m2type_GetBooleanTrue ();
}
...
static tree dorounds (location_t location ATTRIBUTE_UNUSED, tree type ATTRIBUTE_UNUSED)
{
  if (FLT_ROUNDS)
    return m2type_GetBooleanTrue ();
  else
...
static tree dogUnderflow (location_t location ATTRIBUTE_UNUSED, tree type)
{
  if (TREE_CODE (type) == REAL_TYPE)
    {
      enum machine_mode mode = TYPE_MODE (type);
...
static tree doexception (location_t location ATTRIBUTE_UNUSED, tree type ATTRIBUTE_UNUSED)
{
  return m2type_GetBooleanTrue ();
}
...
static tree doextend (location_t location ATTRIBUTE_UNUSED, tree type ATTRIBUTE_UNUSED)
{
  return m2type_GetBooleanTrue ();
}
...
m2builtins_BuildBuiltinTree (location_t location, char *name)
{
  ...
  for (fe = &list_of_builtins[0]; fe->name != NULL; fe++)
    if (strcmp (name, fe->name) == 0) {
      ...
      m2statement_SetLastFunction (m2treelib_DoCall (location, fe->return_node, funcptr, m2statement_GetParamList ()));
...
DoBuiltinMemCopy (location_t location, tree dest, tree src, tree bytes)
{
  ...
  tree call     = m2treelib_DoCall3 (location, ptr_type_node, funcptr, dest, src, bytes);
#if 0
...
static void
create_function_prototype (location_t location, struct builtin_function_entry *fe)
{
  tree ftype;
