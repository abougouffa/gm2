...
/*
 *  gm2_finish_build_array_type - complete building the partially created array type, arrayType.
 *                                The arrayType is now known to be declared as:
 *                                ARRAY index_type OF elt_type.  There will only ever be
 *                                one gcc tree type for this array definition.
 *                                The third parameter, type, is a front end type and this
 *                                is necessary so that the canonicalization creates unique
 *                                array types for each type.
 */

static
tree
gm2_finish_build_array_type (tree arrayType, tree elt_type, tree index_type, int type)
{
  tree old = arrayType;
...
tree
m2type_BuildEndArrayType (tree arraytype, tree elementtype, tree indextype, int type)
{
  ...
  if (TREE_CODE (elementtype) == FUNCTION_TYPE)
    ...
    return gm2_finish_build_array_type (arraytype, m2tree_skip_type_decl (elementtype), indextype, type);
}
 ...
 *                                ARRAY [low..high] OF arrayType.
 *                                this matches the front end data type, fetype,
 *                                which is only used during canonicalization.
 */
...
/*
 *  build_m2_word16_type_node - build an ISO 16 bit word as an ARRAY [0..1] OF loc
 */
...
/*
 *  build_m2_word32_type_node - build an ISO 32 bit word as an ARRAY [0..3] OF loc
 */
...
/*
 *  build_m2_word64_type_node - build an ISO 32 bit word as an ARRAY [0..7] OF loc
 */
...
/* --fixme-- section  check these gaius - the function names are misleading --fixme-- once the compiler is building correctly */
/*
...
build_bitset_type (location_t location)
{
  ...
  return m2type_BuildSetTypeFromSubrange (location,
					  NULL, bitnum_type_node,
                                          m2decl_BuildIntegerConstant (0),
                                          m2decl_BuildIntegerConstant (m2decl_GetBitsPerBitset()-1));
}
#endif
                                 ...
                                 tree lowval, tree highval)
{
  ...
  if (broken_set_debugging_info)
    ...
    tree noelements = m2expr_BuildAdd (location,
				       m2expr_BuildSub (location, highval, lowval, FALSE),
                                       integer_one_node,
                                       FALSE);
    tree settype;
...
/*
 *  build_m2_size_set_type - build and return a set type with, precision, bits.
 */

static tree
build_m2_size_set_type (location_t location, int precision)
{
  tree bitnum_type_node = build_range_type (m2tree_skip_type_decl (m2type_GetCardinalType()),
                                            m2decl_BuildIntegerConstant (0),
...
/*
 *  build_m2_specific_size_type - build a specific data type matching number of
 *                                bits, precision, whether it, is_signed.  It
 *                                creates a set type if base == SET_TYPE
 *                                or returns the already created real, if REAL_TYPE
 *                                is specified.
 */

static
tree
build_m2_specific_size_type (location_t location, enum tree_code base, int precision, int is_signed)
{
  tree c;
  ...
  if (base == REAL_TYPE)
    {
      if (mode_for_size (TYPE_PRECISION (c), MODE_FLOAT, 0) == BLKmode)
	return NULL;
      layout_type (c);
    }
...
tree
m2type_BuildStartFunctionType (location_t location ATTRIBUTE_UNUSED, char *name ATTRIBUTE_UNUSED)
{
...
tree
m2type_BuildVariableArrayAndDeclare (location_t location,
				     tree elementtype, tree high, char *name,
                                     tree scope)
{
  ...
  if (TYPE_VALUES (enumtype) != 0)
    {
      ...
      error_at (location, "redeclaration of enum %qs", IDENTIFIER_POINTER (name));
...
tree
m2type_BuildEnumerator (location_t location, char *name, tree value, tree *enumvalues)
{
  tree id = get_identifier (name);
...
tree
m2type_BuildSetType (location_t location, char *name, tree type, tree lowval, tree highval)
{
...
m2type_BuildEndSetConstructor (void *p)
{
  ...
  constructor = build_constructor_from_list (c->constructor_type,
                                             nreverse (c->constructor_element_list));
...
{
  struct struct_constructor *c = (struct struct_constructor *)p;
  tree constructor = build_constructor_from_list (c->constructor_type,
                                                  nreverse (c->constructor_element_list));
  TREE_CONSTANT (constructor) = 1;
  TREE_STATIC (constructor) = 1;
...
void
m2type_BuildArrayConstructorElement (void *p, tree value,
                                     tree indice)
{
  struct struct_constructor *c = (struct struct_constructor *)p;
...
tree
m2type_BuildArrayStringConstructor (location_t location, tree arrayType, tree str, tree length)
{
  tree n;
  tree val;
  int i=0;
...
m2type_BuildSubrangeType (location_t location, char *name, tree type, tree lowval, tree highval)
{
  ...
  if ((name == NULL) || (strcmp (name, "") == 0)) {
    ...
    id = m2type_DeclareKnownType (location, name, id);
    layout_type (m2tree_skip_type_decl (id));
    return id;
  }
}
...
{
  return build_real (m2type_GetLongRealType(),
...
/*
 *  m2type_BuildStartVarient - builds a varient record.
...
/*
 *  m2type_BuildStartFieldVarient - builds a field varient record.
 *                                  It creates a record field which
 *                                  has a, name, and whose type is a
 *                                  record.
 */
...
m2type_BuildEndRecord (location_t location, tree record, tree fieldlist, int isPacked)
{
  ...
  ASSERT_CONDITION (TYPE_FIELDS (record) == fieldlist);  /* --fixme-- remove this */
  TYPE_FIELDS (record) = fieldlist;  /* and this once the condition always passes */
...
/*
 *  m2type_BuildStartFieldRecord - starts building a field record.
 *                                 It returns the field which must be
 *                                 completed by calling gm2_finish_decl.
 */
...
m2type_BuildStartFieldRecord (location_t location, char *name, tree type)
{
  ...
  field = build_decl (location, FIELD_DECL, declarator, m2tree_skip_type_decl (type)) ;
  return field;
}
...
tree
m2type_SetRecordFieldOffset (tree field, tree byteOffset, tree bitOffset, tree fieldtype, tree nbits)
{
...
/*
 *  gm2_type_for_size - return an integer type with BITS bits of precision,
 *                      that is unsigned if UNSIGNEDP is nonzero, otherwise
 *                      signed.
 */
...
/*
 *  gm2_unsigned_type - return an unsigned type the same as TYPE in other respects.
 */
...
/*
 *  gm2_signed_type - return a signed type the same as TYPE in other respects.
 */
...
static int
check_type (tree baseType, tree type, int unsignedp, tree baseu, tree bases, tree *result)
{
...
/*
 *  gm2_signed_or_unsigned_type - return a type the same as TYPE except unsigned or
...
m2type_gm2_signed_or_unsigned_type (int unsignedp, tree type)
{
  ...
  /*
   *  For INTEGER_TYPEs we must check the precision as well, so as to yield
   *  correct results for bit-field types.
   */

  if (check_type (signed_char_type_node, type, unsignedp, unsigned_char_type_node, signed_char_type_node, &result))
    return result;
  if (check_type (integer_type_node, type, unsignedp, unsigned_type_node, integer_type_node, &result))
    return result;
  if (check_type (short_integer_type_node, type, unsignedp, short_unsigned_type_node, short_integer_type_node, &result))
    return result;
  if (check_type (long_integer_type_node, type, unsignedp, long_unsigned_type_node, long_integer_type_node, &result))
    return result;
  if (check_type (long_long_integer_type_node, type, unsignedp, long_long_unsigned_type_node, long_long_integer_type_node, &result))
    return result;
#if 0
  if (check_type (widest_integer_literal_type_node, type, unsignedp, widest_unsigned_literal_type_node, widest_integer_literal_type_node, &result))
    return result;
#endif

#if HOST_BITS_PER_WIDE_INT >= 64
  if (check_type (intTI_type_node, type, unsignedp, unsigned_intTI_type_node, intTI_type_node, &result))
    return result;
#endif
  if (check_type (intDI_type_node, type, unsignedp, unsigned_intDI_type_node, intDI_type_node, &result))
    return result;
  if (check_type (intSI_type_node, type, unsignedp, unsigned_intSI_type_node, intSI_type_node, &result))
    return result;
  if (check_type (intHI_type_node, type, unsignedp, unsigned_intHI_type_node, intHI_type_node, &result))
    return result;
  if (check_type (intQI_type_node, type, unsignedp, unsigned_intQI_type_node, intQI_type_node, &result))
    return result;
#undef TYPE_OK
