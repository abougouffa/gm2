...
It originated from p2c-1.20 another GNU Project "p2c" file: src/parse.c
*/
...
Static int arecompilingdefn = 0;


int isDefinition ()
{
  return arecompilingdefn == 1;
}
...
int isextern;
{
    ...
    if (func->type->basetype != tp_void || ansiC != 0) {
        outbasetype(func->type, ODECL_FORWARD|ODECL_NOPRES);
        output(" ");
    }
    outdeclarator(func->type, func->name, ODECL_FORWARD|ODECL_NOPRES);
    output(";\n");
    func->wasdeclared = 1;
}
...
#define BR_PROGRAM      0x100   /* program module */
...
int opts, serial;
{
	...
	if (cmtdebug)
		...
		output(name_CASECHECK);
            ...
            case SK_NOCASECHECK:
                output("/* deliberate catch all default statement in CASE */\n");
                break;
     ...
     int sflags;
{
    ...
    if (curctx->kind == MK_FUNCTION) {     /* handle copy parameters */
		...
		if ((mp->kind == MK_VARPARAM) && (mp->type && mp->type->basetype &&
						  (mp->type->basetype->kind == TK_OPAQUE))) {
		  addstmt(SK_ASSIGN);
		  mp->rectype = makepointertype(tp_anyptr);
		  sp->exp1 = makeexpr_assign(makeexpr_var(mp),
					     makeexpr_cast(makeexpr_name(mp->othername, mp->rectype),
							   mp->type->basetype->basetype));
		  mp->refcount++;
		} else if ((mp->kind == MK_PARAM) && (mp->type && mp->type->kind == TK_OPAQUE)) {
		  addstmt(SK_ASSIGN);
		  mp->rectype = tp_anyptr;
		  sp->exp1 = makeexpr_assign(makeexpr_var(mp),
					     makeexpr_name(mp->othername, mp->rectype));
		  mp->refcount++;
                } else if (checkvarchanged(spbody, mp)) {
                    addstmt(SK_ASSIGN);
                    sp->exp1 = makeexpr_assign(makeexpr_var(mp),
     ...
     Meaning *func;
{
  ...
  for (mp = func->type->fbase; mp; mp = mp->xnext) {
    if (mp->kind == MK_VARPARAM) {
      if (mp->type->basetype && (mp->type->basetype->kind == TK_OPAQUE)) {
	/* modula-2 opaque type is used as a parameter */
	mp->othername = stralloc(format_s(name_COPYPAR, mp->name));
	mp->rectype   = mp->type;
      }
    } else if (mp->kind == MK_PARAM) {
      if (mp->type->kind == TK_OPAQUE) {
	mp->othername = stralloc(format_s(name_COPYPAR, mp->name));
	mp->rectype   = mp->type;
      }
    }
...
Static void p_function(isfunc, ismodule)
     int isfunc, ismodule;
{
    Meaning *func;
    Type *type;
...
/*
 *  if inheader == 2 then we have encountered IMPORT module;
 */
	...
	while (curtok == TOK_INTFONLY) {
		...
		case TOK_EXPORT:
		    ...
		    p_nested_module();
                ...
                case TOK_PROCEDURE:
                    p_function(0, (blkind==TOK_PROGRAM) || (blkind==TOK_IMPLEMENT));
                    break;

                case TOK_FUNCTION:
                    p_function(1, (blkind==TOK_PROGRAM) || (blkind==TOK_IMPLEMENT));
                    break;
...
void p_program()
{
    ...
    switch (curtok) {
		...
		outcontext = prog;
		if (modula2 && isdefn == 0)
		  out_block(sp, BR_FUNCTION | BR_PROGRAM, 10000);
		else
		  out_block(sp, BR_FUNCTION, 10000);
