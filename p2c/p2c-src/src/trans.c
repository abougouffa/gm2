...
It originated from p2c-1.20 another GNU Project "p2c" file: src/trans.c
*/
...
/* "p2c", a Pascal to C translator.
   ...
   Author's address: daveg@csvax.caltech.edu; 256-80
...
char **argv;
{
    ...
    *hdrfname = 0;
    ...
    if (modula2 && definition)
      codef = NULL;
    else if (strcmp(codefname, "-")) {
      if (*codefname == 0)
	codef = NULL;
      else {
...
int outmemname(const char *name)
{
    fprintf(stderr, "p2c: out of memory handling %s\n", name);
    exit(EXIT_FAILURE);
}
...
const char *meaningkindname(kind)
...
Type *tp;
{
    ...
    fprintf(outf, "      Type %lx, kind=%s", (long unsigned int)tp, typekindname(tp->kind));
#ifdef HASDUMPS
    fprintf(outf, ", meaning=%lx, basetype=%lx, indextype=%lx\n",
            (long unsigned int)tp->meaning,
	    (long unsigned int)tp->basetype,
	    (long unsigned int)tp->indextype);
    tp->dumped = 1;
    if (tp->basetype)
...
Meaning *mp;
{
    ...
    if (ISBOGUS(mp)) {
	fprintf(outf, "0x%lX\n", (long unsigned int)mp);
	return;
    }
    fprintf(outf, "   Meaning %lx, name=%s, kind=%s",
	    (long unsigned int)mp, ((mp->name) ? mp->name : "<null>"),
	    meaningkindname(mp->kind));
#ifdef HASDUMPS
    fprintf(outf, ", ctx=%lx, cbase=%lx, cnext=%lx, type=%lx\n",
            (long unsigned int)mp->ctx,
	    (long unsigned int)mp->cbase,
	    (long unsigned int)mp->cnext,
	    (long unsigned int)mp->type);
...
int waddr;
{
    ...
    if (ISBOGUS(tp)) {
	fprintf(outf, "0x%lX", (long unsigned int)tp);
	return;
    }
	...
	if (waddr) {
	    fprintf(outf, "%lX", (long unsigned int)tp);
	    if (tp->dumped)
		return;
	    fprintf(outf, ":");
	    tp->dumped = 1;
...
Expr *ex;
{
    ...
    if (ISBOGUS(ex)) {
	fprintf(outf, "0x%lX", (long unsigned int)ex);
	return;
    }
    ...
    if (ISBOGUS(ex->val.s))
	fprintf(outf, "[0x%lX]", (long unsigned int)ex->val.s);
    else if (ex->val.s) {
        switch (ex->kind) {
            ...
            default:
                switch (ex->val.type ? ex->val.type->kind : TK_VOID) {
                    ...
                    case TK_REAL:
                        ...
                        break;
                    default:
                        fprintf(outf, "[s=%lx]", (long unsigned int)ex->val.s);
                }
                break;
        }
    }
