...
It originated from p2c-1.20 another GNU Project "p2c" file: src/pexpr.c
*/
...
Type *target;
{
	...
	bindnames(ex);
        ...
        ex->val.type = mp->type;
    } else if (mp->kind == MK_SPVAR && has_handler(mp)) {
        gettok();
        ex = (*mp->variable_handler_meaning)(mp);
...
int sure;
{
	...
	/* new addition to allow small sets with range say [94..98] */
	if (ord_range(type, &smin, &smax)) {
	  if ((smax-smin < setbits) && (smin > 0))
	    ex = makeexpr_minus(ex, makeexpr_long(smin));
	}
        first[num] = ex;
#else
...
int isGenericArray (tp)
     Type *tp;
{
  /* broken */
  return (modula2 && tp && (tp->kind == TK_ARRAY) && (tp->basetype) &&
	  ((tp->basetype->meaning->type == tp_abyte) ||
	   (tp->basetype->meaning->type == tp_sbyte) ||
	   (tp->basetype->meaning->type == tp_ubyte)));
}
...
int firstarg, ismacro;
{
	...
	isconf = ((tp->kind == TK_ARRAY ||
		...
		else if (tp2 == tp_word && modula2)
  		    ex2 = makeexpr_cast(ex2, tp);
                else if (args->type == tp_word && modula2)
                    ex2 = makeexpr_cast(ex2, tp_word);
                else if (ord_type(tp)->kind == TK_INTEGER && !ismacro)
                    ex2 = makeexpr_arglong(ex2, long_type(tp));
		else if (args->type->kind == TK_OPAQUE)
		    ex2 = makeexpr_cast(ex2, tp_anyptr);
		...
		ex->args[i] = ex2;
		    ...
		    if (args->type && (args->type->basetype) && (args->type->basetype->kind == TK_OPAQUE)) {
		      /* we see whether we need to cast the argument to an opaque (void **) */
		      ex2 = makeexpr_cast(ex2, makepointertype(tp_anyptr));

                    } else if (args->anyvarflag ||
                        (tp->kind == TK_POINTER && tp2->kind == TK_POINTER &&
                         (tp == tp_anyptr || tp2 == tp_anyptr))) {
			if (!ismacro)
			    ex2 = makeexpr_cast(ex2, args->type);
		    } else if (args->type && args->type->basetype == tp_word && modula2) {
                        ex2 = makeexpr_cast(ex2, args->type);
		    } else {
                        if (tp2 != tp && !isconf &&
			    (tp2->kind != TK_STRING ||
	...
	if (isconf &&   /* conformant array or string */
		...
		if (tp2->kind == TK_SMALLARRAY) {
		  ...
		  if (isGenericArray(tp)) {
		    /* ARRAY OF BYTE or ARRAY OF WORD */
		    ex->args[fakenum++] = makeexpr_long(0);
		    ex->args[fakenum++] = makeexpr_minus(makeexpr_div(makeexpr_sizeof(makeexpr_type(tp2), 1),
...
Type *target;
{
	...
	case TOK_IF:    /* nifty Pascal extension */
        ...
        case TOK_IDENT:
			...
			if (mp->function_handler_meaning)
			  ex = (*mp->function_handler_meaning)(mp);
			else if (mp->variable_handler_meaning)
			  ex = (*mp->variable_handler_meaning)(mp);
			else
			  ex = makeexpr_long(0);
...
#if 1
	    /* new addition to allow small sets with range say [94..98] */
	    if (ord_range(ex2->val.type->indextype, &smin, &smax)) {
	      if ((smax-smin < setbits) && (smin > 0))
		ex = makeexpr_minus(ex, makeexpr_long(smin));
	    }
#endif
