...
It originated from p2c-1.20 another GNU Project "p2c" file: src/decl.c
*/
...
const char *name;
{
    ...
    sym = findsymbol(fixpascalname((char *)name));
    for (mp = sym->mbase; mp && mp->ctx != curctx; mp = mp->snext) ;
    if (!mp) {
...
Stmt *(*handler)(Expr *);
{
    ...
    if (!handler) {
      ...
      mp->procedure_handler_meaning = nullspecialproc;
    } else {
      mp = makestandardmeaning(MK_SPECIAL, name);
      mp->procedure_handler_expr = handler;
    }
    return mp;
}
...
Expr *(*handler)(Expr *);
{
    ...
    mp = makestandardmeaning(MK_SPECIAL, name);
    if (handler)
      mp->function_handler_meaning = (void *)handler;
    else
      mp->function_handler_meaning = nullspecialfunc;
...
Expr *(*handler)(Expr *);
{
    ...
    if (mp) {
      if (handler) {
	mp->function_handler_expr = handler;
      } else {
	mp->function_handler_expr = nullstandardfunc;
      }
      if (!mp->isfunction) {
...
Expr *(*handler)(Meaning *);
{
    ...
    mp->variable_handler_meaning = handler;
    return mp;
}
...
int has_handler (mp)
     Meaning *mp;
{
  return( (mp->procedure_handler_expr) ||
	  (mp->procedure_handler_meaning) ||
	  (mp->function_handler_expr) ||
	  (mp->function_handler_meaning) ||
	  (mp->variable_handler_meaning) );
}
...
#if 0
/* testing new function (gaius) not needed */

void unactivatemeaning(mp)
Meaning *mp;
{
    Meaning *mp2;
    ...
    mp->isactive = 0;
    /* move to end of symbol list, providing that it exists */
    mp2=mp->sym->mbase;
    while (mp2 && (mp2->snext != mp))
      mp2 = mp2->snext;
    ...
    /* mp2 is before mp */
    mp2->snext = mp->snext;  /* mp is now unlinked */
    while (mp2->snext != NULL)
      mp2 = mp2->snext;
...
void handle_nameof()
{
    ...
    for (sl = nameoflist; sl; sl = sl->next) {
        ...
        sl2->value = sl->value;
        if (debug > 0)
            fprintf(outf, "symbol %s gets \"%s\" -> \"%ld\"\n",
...
/*    mp->serial = curserial = ++serialcount;    */
    ...
    mp->isactive = 1;
    mp->exported = 0;
    mp->procedure_handler_expr    = NULL;
    mp->procedure_handler_meaning = NULL;
    mp->function_handler_expr     = NULL;
    mp->function_handler_meaning  = NULL;
    mp->variable_handler_meaning  = NULL;
    mp->dumped = 0;
    mp->isreturn = 0;
    mp->fakeparam = 0;
    mp->namedfile = 0;
...
int isalreadydefined (Symbol *sym)
{
  return (sym->mbase && sym->mbase->ctx == curctx && curctx != NULL);
}
...
enum meaningkind kind, namekind;
{
	...
	return;
    if (!mp->exported || !*exportsymbol || (mp->ctx && mp->ctx->language_C))
        symfmt = "";
		...
		if ((mp->kind == MK_VARPARAM) && (tp->basetype->kind == TK_OPAQUE))
		  ...
		  tp = tp_anyptr;  /* force parameters to use void */
			...
			out_type(tp, 1);
		      ...
		      if (modula2 && (mp->kind == MK_PARAM) && (! mp->fakeparam)) {
			if (tp && tp->basetype && tp->basetype->kind == TK_ARRAY) {
			  if (useconsts == 2)
			    output("Const ");
			  else if (ansiC > 0)
			    output("const ");
...
Type *makeopaquetype(type)
     Type *type;
{
    Type *tp;

    tp = maketype(TK_OPAQUE);
    tp->basetype = type;
    return tp;
}
...
Meaning *tname;
{
    ...
    if (tname && tname->type && (tname->type->kind == TK_OPAQUE)) {
      tname->type->basetype = tp;
      tp->meaning = tname;
      movetoend(tname);
      return tname->type;
    }
...
Meaning *mp;
{
    ...
    case TK_OPAQUE:
      if (isDefinition())
	output(format_s("#ifndef %s\n", format_s(name_GSYMBOL, mp->ctx->name)));
      output("/* opaque type declaration */\n");

      output("typedef ");
      mp->type->basetype->preserved = 0;
      outbasetype(mp->type, 0);
      output(" ");
      outdeclarator(mp->type->basetype, mp->name, 0);
      output(";\n");
      mp->type->basetype->preserved = 1;
      outtrailcomment(mp->comments, -1, declcommentindent);
      if (isDefinition())
	output("#endif\n");
      break;
...
void p_typedecl()
{
    ...
    while (curtok == TOK_IDENT) {
      /*
       *  firstly we check to see whether we are defining an opaque type
       */
      if (modula2 &&
...
/*
 * Local variables:
 *  compile-command: "gcc -c -g -O2 -W -Wall -Wwrite-strings -Wstrict-prototypes -Wmissing-prototypes -Wtraditional -pedantic -Wno-long-long -I./../include/ ./decl.c"
 * End:
 */
