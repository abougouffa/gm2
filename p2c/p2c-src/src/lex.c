...
It originated from p2c-1.20 another GNU Project "p2c" file: src/lex.c
*/
...
void pgetline()
{
    ...
    switch (inputkind) {
		...
		if (inbuf[0] == '#' && inbuf[1] == ' ' && isdigit(inbuf[2])) {
		    ...
		    inf_lnum--;
		    while (isspace(*cp)) cp++;
		    if (*cp == '"' && (cp2 = (char *)my_strchr(cp+1, '"')) != NULL) {
			cp++;
			infname = stralloc(cp);
			infname[cp2 - cp] = 0;
		    }
...
int p2c_only, starparen;
{
    ...
    closing = inbufptr;
    while (isspace(*closing))
      closing++;
    if (strncmp("%%%FORWARD%%%", closing, strlen("%%%FORWARD%%%")) == 0)
      strcpy(closing, "*)");
...
void gettok()
{
    ...
    if (tokentrace) {
        ...
        switch (curtok) {
            ...
            case TOK_INTLIT:
            case TOK_MININT:
                printf(", curtokint = %d", (int)curtokint);
                break;
            case TOK_REALLIT:
            case TOK_STRLIT:
...
#endif
{
    ...
    for (;;) {
		...
		if (*inbufptr == '=') {
            ...
            case '%':
	      if (strncmp(inbufptr, "%%FORWARD%%%", strlen("%%FORWARD%%%")) == 0) {
		  strcpy(inbufptr, "(*    *)");
		  gettok();
		  return;
	      }
