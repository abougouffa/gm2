...
It originated from p2c-1.20 another GNU Project "p2c" file: src/comment.c
*/
...
int kind;
{
    ...
    if (eatcomments == 1)
	return;
    for (cp = curtokbuf; (cp = (char *)my_strchr((const char *)cp, '*')) != NULL; ) {
	if (*++cp == '/') {
	    cp[-1] = '%';
...
long serial;
{
	...
	prefix = format_sd("\004 p2c: %s:%d:", infname, inf_lnum);
    len1 = strlen(prefix);
    len2 = strlen(msg) + 2;
...
int okind, ostamp, kind, stamp;
{
	...
	if (matchcomment(cmt, okind, ostamp)) {
	    ...
	    if (cmtdebug > 1)
		fprintf(outf, " to [%s:%d]\n",
			CMT_NAMES[getcommentkind(cmt)], (int)(cmt->value & CMT_MASK));
	}
	cmt = cmt->next;
    }
}
