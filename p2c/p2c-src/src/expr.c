...
It originated from p2c-1.20 another GNU Project "p2c" file: src/expr.c
*/
...
Type *type;
{
    ...
    switch (a->kind) {
	    ...
	    /* check for WORDs which must be cast */
	    if (a->val.type == tp_word)
...
Type *type;
{
    ...
    if ((a->val.type == tp_anyptr) && (type->kind == TK_OPAQUE)) {
...
Expr *a, *b;
{
    ...
    if (a->val.type != b->val.type) {
        ...
        } else if ((a->val.type == tp_word) && (b->val.type != tp_word)) {
            b = gentle_cast(b, tp_word);
	} else if ((b->val.type == tp_word) && (a->val.type != tp_word)) {
	    a = gentle_cast(a, tp_word);
	}
    }
...
int check;
{
    ...
    if (a->val.type == NULL) {
      error("type of expression is null");
      return NULL;
    }
    if (a->val.type->kind == TK_OPAQUE) {
      a->val.type = a->val.type->basetype;
      return makeexpr_hat(a, check);
    }      
    ...
    if (a->val.type->kind == TK_OPAQUE) {
      if (a->val.type->basetype->kind != TK_POINTER) {
        warning("bad pointer dereference [165]");
        return a;
      } else {
	if (a->val.type->basetype == tp_anyptr) {
	  error("failed to resolve opaque type");
	  return a;
	} else
	  return makeexpr_un(EK_HAT, a->val.type->basetype, a);
      }
    } else if (a->val.type->kind != TK_POINTER || !a->val.type->basetype) {
        warning("bad pointer dereference [165]");
        return a;
    }
    if (a->kind == EK_CAST &&
