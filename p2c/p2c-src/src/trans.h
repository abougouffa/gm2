...
It originated from p2c-1.20 another GNU Project file: src/trans.h
*/
...
#define MININT     -2147483647    /* keeps the compiler warnings at bay */
...
const char *p2c_home = P2C_HOME;
#else
extern const char *p2c_home;
#endif
...
#ifdef define_globals
const char *toknames[(int)TOK_LAST] = { "",
...
#ifdef DEFDUMPS
const char *meaningkindnames[(int)MK_LAST] = {
    "MK_NONE", "MK_SPECIAL",
    "MK_MODULE", "MK_FUNCTION", "MK_CONST", "MK_VAR", "MK_TYPE",
    "MK_FIELD", "MK_LABEL", "MK_VARIANT",
    "MK_PARAM", "MK_VARPARAM", "MK_VARREF", "MK_VARMAC",
 ...
 *
 * TK_OPAQUE:  opaque pointer type.
 *    tp->basetype => the pointer which implements an opaque type.
 *                    During construction this might be tp_anyptr.
 *    tp->meaning  => meaning of symbol when opaque was used.
 *
...
#ifdef DEFDUMPS
const char *typekindnames[(int)TK_LAST] = {
    "TK_NONE",
    "TK_INTEGER", "TK_CHAR", "TK_BOOLEAN", "TK_REAL", "TK_VOID",
    "TK_SUBR", "TK_ENUM", "TK_POINTER", "TK_STRING",
    "TK_RECORD", "TK_ARRAY", "TK_SET", "TK_FILE", "TK_FUNCTION",
...
#ifdef DEFDUMPS
const char *exprkindnames[(int)EK_LAST] = {
    "EK_EQ", "EK_NE", "EK_LT", "EK_GT", "EK_LE", "EK_GE",
    "EK_PLUS", "EK_NEG", "EK_TIMES", "EK_DIVIDE",
    "EK_DIV", "EK_MOD",
    "EK_OR", "EK_AND", "EK_NOT",
...
enum stmtkind {
    ...
    SK_HEADER, SK_CASECHECK, SK_BODY, SK_NOCASECHECK,
    SK_LAST
} ;

#ifdef DEFDUMPS
const char *stmtkindnames[(int)SK_LAST] = {
    "SK_ASSIGN", "SK_RETURN",
    "SK_CASE", "SK_CASELABEL", "SK_IF",
    "SK_FOR", "SK_REPEAT", "SK_WHILE", "SK_BREAK", "SK_CONTINUE",
    "SK_TRY", "SK_GOTO", "SK_LABEL",
...
typedef struct S_meaning {
    ...
    Stmt          *(*procedure_handler_expr)(Expr *);   /* Custom translator for procedure */
    Stmt          *(*procedure_handler_meaning)(struct S_meaning *);   /* Custom translator for procedure */
    Expr          *(*function_handler_meaning)(struct S_meaning *); /* Custom translator for function */
    Expr          *(*function_handler_expr)(Expr *); /* Custom translator for function */
    Expr          *(*variable_handler_meaning)(struct S_meaning *);     /* Custom translator for variable */
...
#ifdef define_globals
const char *CMT_NAMES[] = { "DONE", "PRE", "POST", "3", "TRAIL", "5",
                      "BEGIN", "END", "ELSE" };
#else
extern const char *CMT_NAMES[];
#endif
...
#define ALLOC(N,TYPE,NAME) \
    ((alloctemp = malloc((unsigned)((N)*sizeof(TYPE)))), \
     (alloctemp ? (TYPE *) alloctemp : (TYPE *) outmemname(NAME)))
...
#define ALLOC(N,TYPE,NAME)   xmalloc_with_name((unsigned)((N)*sizeof(TYPE)), #NAME)

#define ALLOCV(N,TYPE,NAME)  xmalloc_with_name(((unsigned)(N)), #NAME)

#define REALLOC(P,N,TYPE)    xrealloc((P), (unsigned)((N)*sizeof(TYPE)))

#else
#define ALLOC(N,TYPE,NAME) \
    ((alloctemp = malloc((unsigned)((N)*sizeof(TYPE)))) || outmemname(#NAME), \
     (TYPE *) alloctemp)

#define ALLOCV(N,TYPE,NAME) \
    ((alloctemp = malloc((unsigned)(N))) || outmemname(#NAME), \
     (TYPE *) alloctemp)
