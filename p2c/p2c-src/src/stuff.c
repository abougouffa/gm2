...
It originated from p2c-1.20 another GNU Project "p2c" file: src/stuff.c
*/
...
Static void fatal (str1, str2)
     const char *str1;
     const char *str2;
{
  if (str2 == NULL)
    fprintf (stderr, "p2c: %s\n", str1);
  else
    fprintf (stderr, "p2c: %s (due to %s)\n", str1, str2);
  exit (1);
}
...
void *
xmalloc_with_name (nbytes, name)
     unsigned int nbytes;
     const char *name;
{
  void *value = (void *) malloc (nbytes);
  if (value == 0)
    fatal ("virtual memory exhausted", name);
  return value;
}
...
const char *s;
{
    char *t=stralloc(s);   /* do this first in case **v and *s overlap */
    FREE(*v);
    *v = t;
}
...
const char *fmt, *a1;
{
    return format_gen(fmt, 0L, 0L, 0.0, (char *)a1, NULL, NULL);
}


char *format_ss(fmt, a1, a2)
const char *fmt, *a1, *a2;
{
    return format_gen(fmt, 0L, 0L, 0.0, (char *)a1, (char *)a2, NULL);
}
...
/*
 * Local variables:
 *  compile-command: "gcc -Wall -c -O2 -I./../include/ ./stuff.c"
 * End:
 */
