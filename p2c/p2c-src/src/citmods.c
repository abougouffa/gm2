...
It originated from p2c-1.20 another GNU Project "p2c" file: src/citmods.c
*/
...
extern Stmt *proc_exit(Expr *);    /* from funcs.c */

Static Stmt *proc_return(ex)
     Expr *ex;
{
    return proc_exit(ex);
}
...
Static Expr *func_stdin(ex)
     Expr *ex ATTRIBUTE_UNUSED;
{
    return makeexpr_addr(makeexpr_var(mp_input));
}


Static Expr *func_stdout(ex)
     Expr *ex ATTRIBUTE_UNUSED;
{
...
/*
 * Local variables:
 *  compile-command: "gcc -c -g -O2 -W -Wall -I./../include/ ./citmods.c"
 * End:
 */
