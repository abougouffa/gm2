...
static void sigbusDespatcher (int signum, siginfo_t *info, void *ucontext)
{
  switch (signum) {
...
static void sigfpeDespatcher (int signum, siginfo_t *info, void *ucontext)
{
  switch (signum) {
...
|	(*realvalueProc)(info->si_addr);  /* floating-point overflow */
	...
	(*realvalueProc)(info->si_addr);  /* floating-point inexact result */
      if (info->si_code | FPE_FLTINV)
	(*realvalueProc)(info->si_addr);  /* floating-point invalid result */
