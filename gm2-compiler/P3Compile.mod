...
IMPLEMENTATION MODULE P3Compile ;
...
FROM SymbolTable IMPORT GetCurrentModule, GetSymName ;
...
FROM P3Statement IMPORT StatementSequence ;
...
BEGIN
   ...
   THEN
      ...
      THEN
         (* we BuildProcedureStart inside Block *)
         IF Block()
         THEN
...
(*
   Block - returns TRUE if the block to a procedure was found and parsed.
           Note that Block only refers to a procedure block not a module
           or implementation module or inner module block which all use
           MainBlock.
*)
...
(*
   MainBlock - exactly the same as Block above except that we generate
               a leader quadruple to say this code must go into the initialization
               section of the module.
*)

PROCEDURE MainBlock () : BOOLEAN ;
VAR
   Success: BOOLEAN ;
   ModName: CARDINAL ;
BEGIN
