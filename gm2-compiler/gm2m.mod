...
(*
   Author     : Gaius Mulley
   Title      : gm2m
   Date       : 3/4/86  [$Date: 2002/04/16 21:23:29 $]
   SYSTEM     : GNU Modula-2
...
FROM Strings IMPORT String, InitString, InitStringCharStar, KillString, Add, Length, EqualArray, Mark, string, Dup, Equal, Slice ;
...
BEGIN
   ...
   IF NOT EqualArray(GM2Path, '')
   THEN
      FormatString('-I') ; FormatS(GM2Path) ; FormatString(' ')
   END ;
   INC(CurrentArg) ;
   IF GetArg(s, CurrentArg)
   THEN
...
BEGIN
   ...
   THEN
      ...
      END
   ELSE
      printf2('Usage: %a [-e extension][-o destination directory][-depend][-P %a searchpath][-I compiler searchpath] modulename [options] [compiler]\n', ProgName, ProgName)
   END
END ParseArgs ;
