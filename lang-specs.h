...
/* This is the contribution to the `default_compilers' array in gcc.c for
  ...
  {"@modula-2",
      "%{c:%{Wuselist:%{!Wmodules:%eGNU Modula-2 does not know what to do with -Wuselist given these arguments}} \
           %{!Wmakelist:%{!Wmodules:%{!gm2gcc:%{Wcpp:cc1gm2 -Wcppbegin tradcpp0%s -lang-asm -C %(cpp_unique_options) -Wcppend \
                                               ...
                                               %{!Wcpp:cc1gm2 %(cc1_options) %{f*} %{+e*} %{I*} %{MD} %{MMD} %{M} %{MM} %{MA} %{MT*} %{MF*} %i \
                                                     %{!fsyntax-only:%{!S:-o %{|!pipe:%g.s} |\n\
                                                     as %(asm_options) %{!pipe:%g.s} %A }}}}}} \n\
           %{Wmakelist:%{Wcpp:tradcpp0%s -lang-asm -C %(cpp_unique_options) %g.mod \n\
                              gm2l %{I*} -o %b.lst %g.mod} \n\
                       %{!Wcpp:gm2l %{I*} -o %b.lst %i}} \n\
           %{Wmodules:%{!Wuselist:%{Wcpp:tradcpp0%s -lang-asm -C %(cpp_unique_options) %g.mod \n\
