(* Ulm's Modula-2 Library
   ...
   ----------------------------------------------------------------------------
   $Id: TimeIO.mod,v 1.4 2005/11/21 12:09:59 gaius Exp $
   ----------------------------------------------------------------------------
   $Log: TimeIO.mod,v $
   ...
   * tidied up some ulm implementation modules in ulm-lib-gm2/std
   ...
   Revision 0.4  1999/01/12  18:35:44  borchert
   bug fixes:
   - GetSy used Read instead of NextCh for skipping white space
   - white space belonged to termchars causing it not to be accepted

   Revision 0.3  1997/02/28  15:50:51  borchert
   header fixed

   Revision 0.2  1997/02/21  19:30:02  borchert
   removal of old copyright

   Revision 0.1  1997/02/21  19:18:40  borchert
   Initial revision

   ----------------------------------------------------------------------------
*)

IMPLEMENTATION MODULE TimeIO; (* AFB 9/88 *)
   ...
   *)
   ...
   PROCEDURE RdComp(Read: ReadCharProc; VAR time: TimeField;
	 ...
	 IMPORT WhiteSpace, Read, nl, tab, termCH, CharSet, GetTermChars;
   ...
   MODULE TimePattern;
      ...
      FROM Storage IMPORT ALLOCATE, DEALLOCATE;
      FROM Strings IMPORT StrCpy;
      IMPORT WhiteSpace, Done, CharSet, Components, CompSet;
